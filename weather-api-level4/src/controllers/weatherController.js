...
async function saveWeatherAlert(alertDetails) {
  ...
  try {
    const availableData = await getDataFromDatabase();
    availableData.push(alertDetails);
    await saveDataToDatabase(availableData);
    resolve("Weather alert saved successfully");
  } catch (error) {
    reject(error);
  }
}
